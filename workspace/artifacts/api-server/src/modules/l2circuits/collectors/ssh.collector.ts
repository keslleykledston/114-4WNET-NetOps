...
export async function collectL2CircuitsViaSsh(device: Device): Promise<SSHCollectorOutput> {
  ...
  try {
    const results = await runSSHCommandsForDevice(device, [...L2_SSH_COMMANDS]);
