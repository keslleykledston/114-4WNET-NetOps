...
const ALLOWED_COMMANDS = [
  ...
  new RegExp(`^display bgp vpnv4 vpn-instance \\S+ routing-table peer ${PEER} received-routes$`, "i"),
  new RegExp(`^display bgp vpnv4 vpn-instance \\S+ routing-table peer ${PEER} advertised-routes$`, "i"),
  new RegExp(`^display bgp vpnv6 vpn-instance \\S+ routing-table peer ${PEER} received-routes$`, "i"),
  new RegExp(`^display bgp vpnv6 vpn-instance \\S+ routing-table peer ${PEER} advertised-routes$`, "i"),
  new RegExp(`^display bgp routing-table vpn-instance \\S+ peer ${PEER} received-routes$`, "i"),
  new RegExp(`^display bgp routing-table vpn-instance \\S+ peer ${PEER} advertised-routes$`, "i"),
  // IPv6 variants with VRF (alternative syntax)
  new RegExp(`^display bgp ipv6 vpn-instance \\S+ routing-table peer ${PEER} received-routes$`, "i"),
  new RegExp(`^display bgp ipv6 vpn-instance \\S+ routing-table peer ${PEER} advertised-routes$`, "i"),
  new RegExp(`^display bgp ipv6 routing-table vpn-instance \\S+ peer ${PEER} received-routes$`, "i"),
  new RegExp(`^display bgp ipv6 routing-table vpn-instance \\S+ peer ${PEER} advertised-routes$`, "i"),
  // Display BGP peers (verbose variants)
  new RegExp(`^display bgp vpnv4 vpn-instance \\S+ peer verbose$`, "i"),
  new RegExp(`^display bgp vpnv6 vpn-instance \\S+ peer verbose$`, "i"),
  new RegExp(`^display bgp routing-table peer ${PEER}$`, "i"),
  new RegExp(`^display bgp ipv6 routing-table peer ${PEER}$`, "i"),
  ...
  /^display mpls l2vc$/i,
  /^display mpls l2vc verbose$/i,
  /^display vsi$/i,
  /^display vsi verbose$/i,
  new RegExp(`^display vsi \\S+ verbose$`, "i"),
  /^display mac-address vsi(?:\s+\S+)?$/i,
  /^display mac-address vlan(?:\s+\d+)?$/i,
  /^display vlan summary$/i,
  /^display vlan(?:\s+\d+)?$/i,
  new RegExp(`^display current-configuration interface \\S+$`, "i"),
...
export const HUAWEI_VRP_READONLY_COMMANDS = [
  ...
  "display bgp vpnv4 vpn-instance <VRF> routing-table peer <PEER> received-routes",
  "display bgp vpnv4 vpn-instance <VRF> routing-table peer <PEER> advertised-routes",
  "display bgp vpnv6 vpn-instance <VRF> routing-table peer <PEER> received-routes",
  "display bgp vpnv6 vpn-instance <VRF> routing-table peer <PEER> advertised-routes",
  "display bgp routing-table vpn-instance <VRF> peer <PEER> received-routes",
  "display bgp routing-table vpn-instance <VRF> peer <PEER> advertised-routes",
  "display bgp ipv6 vpn-instance <VRF> routing-table peer <PEER> received-routes",
  "display bgp ipv6 vpn-instance <VRF> routing-table peer <PEER> advertised-routes",
  "display bgp ipv6 routing-table vpn-instance <VRF> peer <PEER> received-routes",
  "display bgp ipv6 routing-table vpn-instance <VRF> peer <PEER> advertised-routes",
  "display bgp vpnv4 vpn-instance <VRF> peer verbose",
  "display bgp vpnv6 vpn-instance <VRF> peer verbose",
  "display bgp routing-table peer <PEER>",
  "display bgp ipv6 routing-table peer <PEER>",
